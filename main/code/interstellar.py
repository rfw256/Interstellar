...
def generate_positions(params):
    ...
    pa_fname = '../../training/positions/sub-%03d_%02dpos.tsv' % (subject, npos)
    
    if not op.exists(pa_fname):
        print("ERROR: Previous positions not found in training data...")
        core.quit()
        
    else:
        print("Found previously generated set of positions, loading...")
        PA_tsv = pd.read_csv(pa_fname, sep = '\t')
        PA_tsv = PA_tsv[PA_tsv.task == params['block']]
        ...
        conds = list(PA_tsv.cond)

    return positions, angles, cues, conds
    

def generate_trials(trainParams, positions, angles, cues, conds, win):
    ...
    block = trainParams['block']
    
    # Set filenames and paths to be used
    tsv_filename_trial = '/sub-wlsubj%03d_run-%02d_LTM_%s_trialdesign.tsv' % (subject, run, time[0])
    subj_dir = op.join('../design/', "sub-wlsubj%03d" % subject)
    
    if not op.exists(subj_dir):
        os.makedirs(subj_dir)
    
    trialnums = list(range(npos)) * ntrials
    if trainParams['debug']: trialnums = list(range(2)) * 5
...
def create_stimuli(params, win, positions, angles, cues):
    # Create stimuli
    ...
    guide = visual.Circle(win, radius = params['ecc'], lineWidth = 1, lineColor = 'lightgray', 
    ...
    if not params['debug']:
        for i, cue_name in enumerate(cues):
            ...
            txt = "Loading stimuli...\n(%d/%d) %s %%" % \
                (i+1, len(cues), int((i+1)/len(cues) * 100))
            msg = visual.TextStim(win, pos=[0, 0], text=txt, units = 'deg')
            msg.draw()
            win.flip()
            cue_stimuli.append(
                visual.ImageStim(win, cue_path, size = 1, units = 'deg'))
    elif params['debug']:
        for i in range(2):
            cue_path = op.join('../../cues', cues[i])
            txt = "Loading stimuli...\n(%d/%d) %s %%" % \
                (i+1, 2, int((i+1)/2 * 100))
            msg = visual.TextStim(win, pos=[0, 0], text=txt, units = 'deg')
            msg.draw()
    ...
    return grating, fixation, guide, correct, sacc, debug, cue_stimuli
    
    
def connect_eyelink(params=trainParams):
    edf_fname = "Is%03dr%02d" % (params['subject'], params['run'])
...
#                        units='deg')
    ...
    win = visual.Window(
        ...
        fullscr = params['fullscreen'], useRetina = params['useretina'])
...
def write_trialdata(trialParams, session_folder, params):
    format_str = "sub-wlsubj%03d_run-%02d_%s_%s_trialdata.tsv"
    filename =  format_str % (params['subject'], params['run'], params['block'], params['time'])
...
def stimulus(fixation, guide, cue, win, scan_clock, parameters, grating, 
             session_folder, edf_file, genv, debug, params, trialParams, el_tracker):
    ...
    # WM | Perception
    elif parameters['block'] in ['wm', 'perception']:
        while scan_clock.getTime() - stim_onsetTime <= cue_dur:
            guide.draw()
            cue.draw()
    ...
    if parameters['block'] == 'perception':
        stim_onsetTime = scan_clock.getTime()
        while scan_clock.getTime() - stim_onsetTime <= delay_dur:
            guide.draw()
            fixation.draw()
...
def run_trial(trialParams, trial_index, grating, fixation, guide, win, scan_clock,
    ...
    # Stimulus
    print("STIMULUS")
    stimulus(fixation, guide, cues[t], win, scan_clock, parameters, grating, 
             session_folder, edf_file, genv, debug, params, trialParams, el_tracker)

    # Saccade Response
    SRT, sac_end_pos = saccade(fixation, guide, win, scan_clock, parameters, 
                               el_tracker, genv, session_folder, edf_file, params,
                               mon, debug, trialParams)

    accuracy = check_saccade(sac_end_pos, parameters['gratingAng'], 
                             params['ecc'], np.degrees(angles), 2)
    
    # Trial end
    el_tracker.sendMessage('TRIAL_RESULT %d' % pylink.TRIAL_OK)
    
    return SRT, sac_end_pos, accuracy
...
def run(params=trainParams):
    ...
    positions, angles, cues, conds = generate_positions(params)
    trialParams = generate_trials(params, positions, angles, cues, conds, win)
    
    # Create stimuli
    grating, fixation, guide, correct, sacc, debug, cues = create_stimuli(
        params, win, positions, angles, cues)
    
    # Display instructions
    instructions = "Press SPACEBAR to move on."
    msg = visual.TextStim(win, pos=[0, 5], text=instructions, units = 'deg')
